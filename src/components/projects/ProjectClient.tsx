        ...
        <section className="pb-24 px-6 md:px-12">
            ...
            <div className="mb-20 space-y-12 max-w-7xl mx-auto">
                ...
                <div className="flex flex-col gap-10">
                    ...
                    <div className="flex flex-col gap-5">
                        ...
                        <span className="text-xs font-black uppercase tracking-[0.3em] text-black/60 text-center md:text-left">Sector Selection</span>
                            ...
                            <div className="flex md:flex-wrap items-center gap-x-10 gap-y-4 min-w-max md:min-w-0 mx-auto md:mx-0">
                                {categories.map((cat) => (
                                    ...
                                    >
                                        ...
                                        {cat}
                                        <span className="ml-0.5 text-xs opacity-40 font-medium tracking-normal">({getCount('category', cat)})</span>
                                        {activeCategory === cat && (
                                            <motion.div 
                                                layoutId="activeCat"
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between border-t border-black/5 pt-8 gap-8">
                        ...
                        <div className="flex flex-col items-center md:items-start gap-4">
                            <span className="text-xs font-black uppercase tracking-[0.3em] text-black/60">Project Status</span>
                            <div className="flex flex-wrap justify-center gap-3">
                                {statuses.map((s) => (
                                    <button
                                        ...
                                        className={clsx(
                                            "text-xs font-medium uppercase tracking-widest transition-all px-6 py-2.5 rounded-sm border flex items-center gap-2",
                                            activeStatus === s 
                        ...
                        {(activeCategory !== "All" || activeStatus !== "All" || searchQuery !== "") && (
                            <motion.button
                                ...
                                className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-black/40 hover:text-black transition-colors group"
                            >
            ...
            <div className="max-w-7xl mx-auto mb-12 flex justify-between items-end border-b border-black/5 pb-8">
                <div className="space-y-1">
                    <span className="text-xs font-black uppercase tracking-[0.3em] text-black/30">Showing Results</span>
                    <h3 className="text-xl font-heading">
                        {filteredProjects.length} {filteredProjects.length === 1 ? 'Record' : 'Architectural Records'}
                    </h3>
                </div>
                <div className="hidden md:block text-xs font-bold uppercase tracking-[0.2em] text-black/20">
                ...
                <AnimatePresence mode="popLayout">
                    {filteredProjects.length > 0 ? (
                            ...
                            {filteredProjects.map((project) => (
                                ...
                                >
                                    <Link href={`/projects/${project.slug}`} className="block">
                                        <div className="relative aspect-[16/10] overflow-hidden mb-8 rounded-none bg-gray-100 shadow-sm transition-all duration-700 group-hover:shadow-2xl group-hover:shadow-black/10">
                                            <Image
                                                src={project.image}
                                                alt={project.title}
                                                fill
                                           ...
                                           {project.status && (
                                            <div className="absolute top-4 right-4 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                                                <span className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest text-black shadow-sm">
                                                    {project.status}
                                                </span>
                                            </div>
                                           )} 
                                        ...
                                        <div className="flex justify-between items-start border-b border-black/5 pb-6 group-hover:border-black/20 transition-colors duration-500">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <span className="text-xs font-bold uppercase tracking-widest text-black/40">{project.category}</span>
                                                    <span className="w-1 h-1 rounded-full bg-black/10" />
                                                    <span className="text-xs font-bold uppercase tracking-widest text-black/40">{project.year}</span>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <span className="text-xs font-bold uppercase tracking-widest text-black/30 block mb-1">Location</span>
                                                <span className="text-xs font-bold uppercase tracking-widest text-black">{project.location}</span>
                                            </div>
                                        </div>
                                    </Link>
                                </motion.div>
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-heading text-black mb-4">No projects found</h3>
                            <p className="text-black/50 max-w-sm mb-8">We couldn't find any projects matching your current filters or search query.</p>
                            <button
                                onClick={resetFilters}
                                className="px-8 py-3 bg-black text-white rounded-sm text-xs font-medium uppercase tracking-widest hover:scale-105 transition-transform"
                            >
                                Reset Filters
                            </button>
                        </motion.div>
...
export default function ProjectClient({ initialProjects = projects }: ProjectClientProps) {
    return (
        <main className="min-h-screen bg-verdant-bg relative">
            ...
            <Suspense fallback={<div className="py-20 text-center uppercase tracking-widest text-xs opacity-50">Loading Projects...</div>}>
