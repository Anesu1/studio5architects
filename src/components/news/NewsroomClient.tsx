                ...
                <div className="flex flex-col lg:flex-row items-center justify-between gap-8 max-w-6xl mx-auto">
                    ...
                    <form className="flex w-full lg:w-auto gap-3" onSubmit={(e) => e.preventDefault()}>
                        <input
                            ...
                            className="bg-white border border-black/10 px-6 py-3 rounded-sm text-xs w-full lg:w-80 focus:outline-none focus:border-black transition-colors"
                        />
                        <button
                            type="submit"
                            className="bg-verdant-black text-white px-8 py-3 rounded-sm text-xs font-medium uppercase tracking-widest whitespace-nowrap hover:scale-105 transition-transform"
                        >
                            Subscribe
                        </button>
                    </form>
                        ...
                        {newsItems.map((item, index) => (
                            ...
                            >
                                <div className="flex items-center gap-4 mb-4">
                                    <span className="text-xs font-bold uppercase tracking-widest bg-black/5 px-2 py-1 rounded text-black/60">{item.category}</span>
                                    <span className="text-xs font-bold uppercase tracking-widest text-black/40">{item.publishedAt}</span>
                                </div>
                                <Link href={`/blogs/${item.slug}`} className="block group-hover:opacity-70 transition-opacity">
                                    <h2 className="text-2xl md:text-3xl font-heading mb-4 leading-tight">
                                        {item.title}
                                    </h2>
                    ...
                    <aside className="w-full lg:w-80 space-y-12">
                        <div className="bg-white p-8 border border-black/5 rounded-sm">
                            ...
                            <div className="space-y-6">
                                <div>
                                    <p className="text-xs uppercase tracking-widest text-black/40 mb-1">General Inquiries</p>
                                    <p className="text-sm font-medium">press@studio5.co.zw</p>
                                </div>
                                <div>
                                    <p className="text-xs uppercase tracking-widest text-black/40 mb-1">Media Relations</p>
