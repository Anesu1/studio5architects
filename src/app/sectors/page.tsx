      ...
      <section style={{ background: '#FFFFFF', paddingTop: '120px' }} className="pb-20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <h1 className="font-serif text-5xl md:text-7xl font-bold text-[#0A0A0A] leading-tight mb-8">
            ...
            <br />
            Every <em style={{ color: 'var(--blue)' }}>Context.</em>
          </h1>
          <p className="text-gray-500 text-lg leading-relaxed max-w-2xl">
            We bring specialized knowledge to every building typology. From complex institutional
            infrastructure and healthcare to bespoke residential architecture and heritage
            conservation.
          </p>
        </div>
      </section>
      ...
      <section className="py-24 bg-[#F4F5F7]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-gray-200">
            {SECTORS.map(({ title, description, filter }) => (
              <div
                key={filter}
              ...
              >
                ...
                <a
                  href={`/portfolio?filter=${filter}`}
                  className="text-blue-600 text-[10px] font-bold tracking-widest uppercase"
                >
                  View Projects →
                </a>
              </div>
